...
auto PlatformMacOS::CreateFrameImpl(
  const std::string& title,
  std::pair<std::size_t, std::size_t> dimensions
) -> MacOSFrameHandle {
...
auto PlatformMacOS::CreateFrameImpl(
  ...
  const NSScreen *main_screen = [NSScreen mainScreen];
  NSRect visible_frame = [main_screen visibleFrame];

  return CreateFrameImpl(
    title,
    {static_cast<std::size_t>(visible_frame.size.width),
     static_cast<std::size_t>(visible_frame.size.height)}
  );
}
