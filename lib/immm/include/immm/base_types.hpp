#ifndef IMMM_BASE_TYPES_HPP
#define IMMM_BASE_TYPES_HPP
...
struct Vec2 {
  ...
  explicit constexpr Vec2(float x = 0.0F, float y = 0.0F) noexcept : x_(x), y_(y) {}

  [[nodiscard]] constexpr auto X() const noexcept -> float { return x_; }
  [[nodiscard]] constexpr auto X() noexcept -> float& { return x_; }

  [[nodiscard]] constexpr auto Y() const noexcept -> float { return y_; }
  [[nodiscard]] constexpr auto Y() noexcept -> float& { return y_; }
...
struct Color {
  ...
  constexpr Color(std::uint8_t red, std::uint8_t green, std::uint8_t blue,
                  std::uint8_t alpha = std::numeric_limits<std::uint8_t>::max()) noexcept
      : Alpha(alpha), Blue(blue), Green(green), Red(red) {}
  // NOLINTEND(bugprone-easily-swappable-parameters)

  // Boy I love everything being constexpr.
  consteval Color(std::string_view hexColor) : Alpha(std::numeric_limits<std::uint8_t>::max()) {
    if (hexColor[0] != '#') {
